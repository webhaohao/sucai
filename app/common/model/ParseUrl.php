...
class ParseUrl
{
    ...
    public function get_nipic_com($verify, $access)
    {
        ...
        preg_match_all('/<a href="javascript:;" onclick="return false;" target="_self" data-line="(.*?)" class="mb5 download-link-btn download-link-btn4 J-download" title="(.*?)">/', $html, $line);
        $download = [];
    ...
    public function get_588ku_com()
    {
        ...
        switch ($url_type) {
            case 'ycpng':
                //PNG
                $curl   = new HttpCurl('http://588ku.com/?m=element&a=down&id=' . $site_code['1'], 'GET', null, $header, true, $this->cookie['cookie_id']);
                ...
                //PSD
                $curl   = new HttpCurl('http://588ku.com/?m=element&a=downpsd&id=' . $site_code['1'], 'GET', null, $header, true, $this->cookie['cookie_id']);
                $result = $curl->send_request()->get_response_body();
                $result = json_decode($result, true);
                if (!empty($result['url'])) {
            ...
            case 'ycbeijing':
                //JPG
                $curl   = new HttpCurl('http://588ku.com/?m=back&a=down&id=' . $site_code['1'], 'GET', null, $header, true, $this->cookie['cookie_id']);
                $result = $curl->send_request()->get_response_body();
                $result = json_decode($result, true);
                if (!empty($result['url'])) {
                ...
                //PSD
                $curl   = new HttpCurl('http://588ku.com/?m=back&a=downpsd&id=' . $site_code['1'], 'GET', null, $header, true, $this->cookie['cookie_id']);
                $result = $curl->send_request()->get_response_body();
                $result = json_decode($result, true);
                if (!empty($result['url'])) {
                    $download['下载源文件'] = $result['url'];
    ...
    public function get_ibaotu_com($verify = '')
    {
        ...
        preg_match('/<div class="dl-btn-wrap clearfix"><a href="(.*?)" id="downvip".*?>.*?VIP免费下载.*?<\/a>/', $html, $url);
    ...
    public function get_tukuppt_com()
    {
        ...
        $header = [
            ...
            'Referer'          => 'http://www.tukuppt.com/muban/' . $site_code['1'] . '.html',
            'X-Requested-With' => 'XMLHttpRequest',
        ];
        $curl   = new HttpCurl('http://www.tukuppt.com/index/down?callback=jQuery&pid=' . $site_code['1'] . '&code=&ispng=0', 'GET', null, $header, true, $this->cookie['cookie_id']);
    ...
    public function get_88tph_com()
    {
        ...
        $postfields = 'doc=' . $site_code['1'] . '&vip=true';
        $header     = [
            'Accept'           => 'application/json, text/javascript, */*; q=0.01',
            'Accept-Encoding'  => 'gzip, deflate, br',
            'Content-Length'   => strlen($postfields),
