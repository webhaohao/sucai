	...
	<div class="collapse navbar-collapse">
		<ul class="navbar-nav mr-auto">
			<li class="nav-item {if $Request.controller == 'Index' && $Request.action == 'index'}active{/if}">
				<a class="nav-link" href="{:url('index/index/index')}">资源解析</a>
			</li>
		...
		<div class="navbar-nav">
			...
			{if $_G['uid']}
				...
				<li class="nav-item"><a class="nav-link" href="{:url('index/account/logout')}">安全退出</a></li>
			{else}
				<li class="nav-item"><a class="nav-link" href="{:url('index/account/login')}">登录</a></li>
				{if $_G['setting']['allow_register']}
					<li class="nav-item"><a class="nav-link" href="{:url('index/account/register')}">注册</a></li>
				{/if}
			{/if}
		</div>
	</div>
